                                 ...
                                 const sensor_msgs::msg::Imu &msg)
{
    ...
    double dt = rclcpp::Time(imu_integration_queue_[1].header.stamp).seconds() -
...
void SPARKFastLIO2::mapIncremental()
{
    ...
    for (int i = 0; i < feats_down_size_; ++i)
    {
        ...
        if (!nearest_points_[i].empty() && flg_EKF_inited_)
        {
            ...
            mid_point.y = std::floor(feats_down_world_->points[i].y / filter_size_map_min_) *
                              filter_size_map_min_ +
                          0.5 * filter_size_map_min_;
            mid_point.z = std::floor(feats_down_world_->points[i].z / filter_size_map_min_) *
                              filter_size_map_min_ +
                          0.5 * filter_size_map_min_;
