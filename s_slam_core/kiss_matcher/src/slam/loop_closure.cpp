...
LoopCandidates LoopClosure::getLoopCandidatesFromQuery(const PoseGraphNode &query_frame,
...
{
    RegOutput reg_output;
    if (query_idx < keyframes.size() && match_idx < keyframes.size())
    {
...
pcl::PointCloud<PointType> LoopClosure::getDebugCloud()
{
    return *debug_cloud_;
}
