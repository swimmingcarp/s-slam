...
void ROBINMatching::match(const std::string& robin_mode, float tuple_scale, bool use_ratio_test)
{
    ...
    tbb::parallel_for(
        ...
        [&](tbb::blocked_range<size_t> r)
        {
            ...
            {
                searchKDTree(
                    feature_tree_i, features_[fj_][j], corres_K[j], dis_j[j], num_candidates);
                bool is_over_ratio =
                    use_ratio_test ? dis_j[j][0] > thr_ratio_test_ * dis_j[j][1] : false;
                                 ...
                                 const float /*tuple_scale*/)
{
    if (!corres.empty())
    {
        ...
        for (size_t i = 0; i < corres_tuple.size(); ++i)
        {
            if (swapped_)
            {
                corres_out.emplace_back(
                    std::pair<int, int>(corres_tuple[i].second, corres_tuple[i].first));
            }
            else
            {
                corres_out.emplace_back(
